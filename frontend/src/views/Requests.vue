...
const selectedUserName = computed(
  () => currentUser.value?.name ?? 'No user selected'
);

const canApprove = (request: { status: RequestStatus }) => {
  return (
    currentUser.value?.role === 'validator' && request.status === 'pending'
  );
};
...
const submit = async () => {
  ...
  if (!currentUser.value) {
    submitError.value =
      'Please select a connected user before creating a request.';
    return;
  }
  ...
  <section class="page-shell">
    <div class="page-header">
      <div>
        ...
        <p class="muted">
          Connected user: <strong>{{ selectedUserName }}</strong>
        </p>
      </div>
      <div class="status-pill" v-if="currentUser?.role">
        {{ currentUser?.role }}
      </div>
    </div>
    ...
    <div class="card requests-dashboard">
      <div class="dashboard-header">
        ...
        <p class="muted">
          Only a validator can approve or reject pending requests.
        </p>
      </div>
